...
max_rhat <- max(mod$BUGSoutput$summary[,"Rhat"]); max_rhat
mod$BUGSoutput$summary[mod$BUGSoutput$summary[,"Rhat"]>1.01,]
mcmc.array <- mod$BUGSoutput$sims.array

#png(here("fig","bad_traceplot.png"))
...
# did not converge? update jags fit 
# if(max_rhat > 1.01){
#   recompile(mod)
#   mod.upd <- update(mod, n.iter=100)  
# }
# check update for convergence
# max(mod.upd$BUGSoutput$summary[,"Rhat"])
# mod$BUGSoutput$summary[mod$BUGSoutput$summary[,"Rhat"]>1.01,]
...
write_csv(p_all, file = here("output", paste0(origin_country, "_results_ci.csv")))
...
mod <- jags.parallel(data = jags.data,
                     parameters.to.save=parnames,
                     n.iter = 100000,
                     model.file = here("code/models","model_usa_natural_no_fb_2.txt"))
...
write_csv(p_all, file = here("output", paste0(origin_country, "_results_nf_ci.csv")))
